...
test("copies the source and temporarily reports success", async () => {
  render(<CodeBlockCopyButton className="absolute" code="const value = 42;" />);
...
test("reports clipboard failures and allows a successful retry", async () => {
  ...
  await act(async () => {
    fireEvent.click(screen.getByRole("button", { name: "Copy failed, retry" }));
    await Promise.resolve();
  });
...
test("ignores a pending clipboard result after unmount", async () => {
  ...
  writeText.mockImplementationOnce(() => pendingWrite.promise);
  const { unmount } = render(<CodeBlockCopyButton code="const value = 42;" />);
